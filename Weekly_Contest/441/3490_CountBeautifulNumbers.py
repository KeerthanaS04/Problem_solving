...
class Solution:
    def beautifulNumbers(self, l: int, r: int) -> int:
        @lru_cache(None)
        def dp(s: str, i: int, tight: bool, isLeadingZero: bool, hasZero: bool, sum: int, prod: int) -> int:
            if i==len(s):
                if isLeadingZero:
                    return 0
            ...
            res = 0
            max_digit = int(s[i]) if tight else 9

            for d in range(max_digit):
                nextTight = tight and (d==max_digit)
                ...
                res+=dp(s, i+1, nextTight, nextIsLeadingZero, nextHasZero, sum+d, nextProd)
            return res
